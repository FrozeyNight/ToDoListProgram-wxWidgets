...
void MainFrame::OnClearButtonClicked(wxCommandEvent& evt){
    ...
    wxMessageDialog clearMessage(this, "Are you sure you want to clear all tasks?", "Clear", wxYES_NO | wxCANCEL);
    int userChoice = clearMessage.ShowModal();
...
void MainFrame::OnWindowClosed(wxCloseEvent& evt){
    wxMessageDialog clearMessage(this, "Do you want to save the tasks?", "Save", wxYES_NO | wxCANCEL);
    int userChoice = clearMessage.ShowModal();
    if(userChoice == wxID_YES){
    ...
    evt.Skip(); // you MUST call this at the end of any wxCloseEvent& method or your app won't close
}
...
void MainFrame::DeleteSelectedTask(){
    int selectedTaskIndex = checkListBox->GetSelection();

    if(selectedTaskIndex == wxNOT_FOUND){
        return;
    }

    checkListBox->Delete(selectedTaskIndex);
}
...
void MainFrame::MoveSelectedTask(int offset){
    ...
    int newIndex = selectedTaskIndex - offset;

    if(newIndex >= 0 && newIndex < checkListBox->GetCount()){
        SwapTasks(selectedTaskIndex, newIndex);
